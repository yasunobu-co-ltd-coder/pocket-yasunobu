...
- opinionsは発言ベースで具体的に
- qAndAは議論の中核的な背景を理解するために必要な質疑応答のみを厳選して抽出する。以下は除外すること:
  - すでに決定事項(conclusion)に反映済みの内容
  - 雑談・確認程度の軽いやりとり
  - 単なる相槌や同意（「そうですね」「はい」等）
  残すべきは「なぜその結論に至ったか」「どんな懸念や論点があったか」が分かるやりとり。該当がなければ空配列
...
・質疑応答: 会議中の質問と回答のやりとりを時系列で残す（以下の形式）
  ...
  ※議論の中核的な背景が分かるQ&Aのみ厳選して記載する
  ※以下は除外: すでに結論・決定事項に反映済みの内容 / 雑談・確認程度の軽いやりとり / 単なる相槌や同意
  ※残すべきは「なぜその結論に至ったか」「どんな懸念や論点があったか」が伝わるやりとり
...
（議題ごとに●で区切る。各議題は上記の項目を含めること。該当がない項目は省略可。質疑応答は議論の背景理解に必要なものだけ残し、決定事項と重複する内容や雑談は省く）
...
- 質疑応答は議論の背景理解に必要なもののみ厳選して残す。決定事項に既に反映された内容や雑談レベルのやりとりは除外する
- expandedTopicsのqAndAデータのうち、結論に至った経緯や懸念事項の背景が分かるものだけを記載する
