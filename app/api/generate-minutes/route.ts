...
const STYLE_PROMPTS: Record<MinutesStyle, string> = {
    meeting: `あなたは音声文字起こしテキストを整形する専門家です。
...
ステップ2: ...`,
    ...
    lecture: `あなたは音声文字起こしテキストを整形する専門家です。
音声認識による誤変換・脱落・繰り返しを含む生テキストを受け取り、
講演・セミナーの記録として読みやすく整理されたテキストを生成してください。

${BASE_RULES}

## 出力形式（講演・セミナー記録）

一人の話者が長く話す講演・講義・セミナー・勉強会向け。

- 冒頭に日時・講演者・テーマ（読み取れる範囲で）を記載。読み取れない項目は「不明」と記載。
- テーマごとに「▶ 見出し」で区切る（見出しは5〜15文字程度）
- 話者の語り口調を活かしつつ、文章として読めるレベルに整える
- 質疑応答パートがある場合、Q: / A: 形式で記録
- 末尾に「要点まとめ」を箇条書きで付ける
...
- ...`,
    ...
    sales: `あなたは音声文字起こしテキストを整形する専門家です。
音声認識による誤変換・脱落・繰り返しを含む生テキストを受け取り、
営業・商談の記録として読みやすく整理されたテキストを生成してください。
...
- 冒頭に日時・参加者（営業側/顧客側）・訪問先・目的を記載。読み取れない項目は「不明」と記載。
- 商談の流れに沿ってセクション分け
- 顧客の発言・要望・懸念点は特に正確に記録する
- 末尾に以下を必ず付ける:
  【顧客ニーズ・課題】顧客が言及した課題や要望の箇条書き
  【提案内容】営業側が提案した内容
  【顧客の反応・温度感】前向き/検討中/懸念あり 等
  【次のアクション】誰が何をいつまでに
  【見積・金額メモ】言及があった場合のみ
...
- ...`,
    ...
    discussion: `あなたは音声文字起こしテキストを整形する専門家です。
音声認識による誤変換・脱落・繰り返しを含む生テキストを受け取り、
対談・インタビューの記録として読みやすく整理されたテキストを生成してください。

${BASE_RULES}

## 出力形式（対談・インタビュー記録）

2人以上の長い対話・インタビュー・ディスカッション向け。

- 冒頭に日時・参加者・テーマ（読み取れる範囲で）を記載。読み取れない項目は「不明」と記載。
- 話題の転換点で「▶ 見出し」を付ける（5〜15文字程度）
- 発言者が特定できる場合は「○○:」形式で発言者を明記
- 対話の流れ・文脈を重視し、やりとりのニュアンスを残す
- 長い発言は要約しつつ要点を維持
- 末尾に「対談のポイント」を箇条書きで付ける
...
function getSystemPrompt(style: MinutesStyle): string {
    return STYLE_PROMPTS[style] || STYLE_PROMPTS.meeting;
}

// ─── CALL1: 議事録テキスト生成 ───

async function generateMinutesSummary(transcript: string, termPrompt: string, style: MinutesStyle = 'meeting'): Promise<string> {
    console.log(`[MINUTES] CALL1: 議事録テキスト生成 開始 (Claude, style=${style})`);
    const styleLabel = { meeting: '会議議事録', lecture: '講演記録', sales: '商談記録', discussion: '対談記録' }[style] || '会議議事録';
    const userMessage = `output_format: ${style}

以下の文字起こしテキストを整形して${styleLabel}を作成してください。
...
export async function POST(req: NextRequest) {
    try {
        ...
        let userId = '';
        let customer = '';
        let style: MinutesStyle = 'meeting';
        ...
        if (contentType.includes('application/json')) {
            ...
            if (['meeting', 'lecture', 'sales', 'discussion'].includes(body.style)) {
                style = body.style;
            }
        } else {
        ...
        console.log(`[MINUTES] 開始: 文字起こし ${transcript.length}文字, ${chunkCount || 1}チャンク, style=${style}`);

        // CALL1: 議事録テキスト生成（Claude）
        const summary = await generateMinutesSummary(transcript, termPrompt, style);
